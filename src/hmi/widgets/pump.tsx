...
export default function Pump({ widget, theme, sim }: WidgetView) {
  ...
  const fill = faulted ? theme.alarm : running ? theme.running : theme.stopped
  return (
    <g>
      {/* mounting base */}
      <rect x={cx - r - 2} y={cy + r - 1} width={2 * r + 4} height={6} rx={2} fill={theme.equipStroke} />
      <circle cx={cx} cy={cy} r={r} fill={fill} stroke={faulted ? theme.alarm : theme.equipStroke} strokeWidth={2}
        className={faulted ? 'hmi-blink' : undefined} data-fault={faulted || undefined} />
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#00000030" strokeWidth={5} />
      <g className={running ? (starting ? 'hmi-spin hmi-blink' : 'hmi-spin') : undefined} style={{ transformOrigin: `${cx}px ${cy}px` }}>
