...
const nameOf = (n: PlantNode) =>
  n.tag ? formatTag(n.tag, '-') : n.label?.trim() || getSymbol(n.symbolId).name
...
function referencesFor(doc: ProjectDoc, node: PlantNode): { sheets: Ref[]; loop: Ref[]; lines: Ref[]; hmi: Ref[] } {
  ...
  const sheets: Ref[] = sheet ? [{ key: sheet.id, label: sheet.name, sub: sheet.drawingNumber || 'no drawing №' }] : []
  ...
  if (sheet) {
    for (const e of sheet.edges) {
      ...
      lines.push({
        ...
        sub: otherNode ? `to ${nameOf(otherNode)}` : 'free end',
        go: () => locateCell(e.id),
      })
    }
  }
  ...
  const hmi: Ref[] = tagText
    ? doc.hmiScreens
        ...
        .map((sc) => ({ key: sc.id, label: sc.name, sub: 'HMI screen' }))
    : []
...
export default function InspectorWhereUsed({ node }: { node: PlantNode }) {
  ...
  return (
    <div className="used">
      ...
      <Group title="Sheet" refs={sheets} empty="not placed on a sheet" />
      <Group title="Loop" refs={loop} empty={node.tag ? 'no other instrument shares this loop' : 'untagged — no loop'} />
      <Group title="Connected lines" refs={lines} empty="nothing connected yet" />
      <Group title="HMI" refs={hmi} empty="not on an operator screen" />
    </div>
  )
}
